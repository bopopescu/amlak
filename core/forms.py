...
class SearchForm(forms.Form):
    ...
    search = forms.CharField(widget=forms.TextInput(attrs={'class':'form-control form-control-sm rounded bright', 'placeholder':'جستجو1'}))
    

class UserLoginForm(forms.Form):
    username = forms.CharField(max_length=100,
                                widget=forms.TextInput(attrs={'class':'form-control', 'placeholder':'نام کاربری خود را وارد نمایید'}))
    password = forms.CharField(widget=forms.PasswordInput(attrs={'class':'form-control', 'placeholder':'کلمه عبور خود را وارد نمایید'}))
...
class UserRegisterForm(forms.ModelForm):
    ...
    password2 = forms.CharField(widget=forms.PasswordInput(attrs={'class':'form-control', 'placeholder':'تکرار کلمه عبور'}))
...
class MelkForm(forms.ModelForm):
    ...
    melk_year = forms.DateTimeField(widget= forms.DateTimeInput(attrs={'class':'single-daterange form-control', 'placeholder':'تاریخ سند'}))
    melk_price = forms.CharField(widget= forms.NumberInput(attrs={'class':'form-control', 'placeholder':'قیمت ملک'}))
    melk_arseh = forms.CharField(widget= forms.NumberInput(attrs={'class':'form-control', 'placeholder':'عرصه ملک'}))
    melk_ayan = forms.CharField(widget= forms.NumberInput(attrs={'class':'form-control', 'placeholder':'اعیان ملک'}))
    melk_comment = forms.CharField(widget= forms.Textarea(attrs={'class':'form-control','rows':'1', 'placeholder':'توضیحات سند'}))
    ...
    post_code = forms.CharField(widget= forms.TextInput(attrs={'class':'form-control', 'placeholder':'کدپستی'}))
    address = forms.CharField(widget= forms.TextInput(attrs={'class':'form-control', 'placeholder':'آدرس'}))
    melk_gps = forms.CharField(widget= forms.TextInput(attrs={'class':'form-control', 'placeholder':'موقعیت جغرافیایی'}))
    
    class Meta:
        model = Melk
        fields = ['melk_name', 'khayer_name','sanad_asli','sanad_farye','sanad_type','melk_karbari','melk_motavali','melk_state',
                    'melk_year','melk_price','melk_arseh','melk_ayan','melk_comment','melk_pic','ostan','city','rosta','post_code','address','melk_gps']
